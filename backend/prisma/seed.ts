...
import { MIN_BCRYPT_ROUNDS } from '../src/common/password-policy';
...
async function main() {
  ...
  const rounds = Math.max(
    Number(process.env.BCRYPT_ROUNDS ?? MIN_BCRYPT_ROUNDS),
    MIN_BCRYPT_ROUNDS,
  );

  const userCol = db.collection<any>('User');
  const categoryCol = db.collection<any>('Category');
  const pocCol = db.collection<any>('POC');
  const feedbackCol = db.collection<any>('Feedback');
  const activityCol = db.collection<any>('ActivityLog');
  const notificationCol = db.collection<any>('AdminNotification');
  const settingsCol = db.collection<any>('PlatformSetting');
  ...
  const developer = await userCol.findOne<{ _id: string }>({ email: 'developer@gmail.com' });
  const regularUser = await userCol.findOne<{ _id: string }>({ email: 'user@gmail.com' });
  const admin = await userCol.findOne<{ _id: string }>({ email: 'admin@gmail.com' });
  const aiCategory = await categoryCol.findOne<{ _id: string }>({ name: 'AI & Machine Learning' });
  const securityCategory = await categoryCol.findOne<{ _id: string }>({ name: 'Cybersecurity' });
  ...
  const chatbot = await pocCol.findOne<{ _id: string }>({ slug: 'ai-chatbot-poc' });
  const securefin = await pocCol.findOne<{ _id: string }>({ slug: 'securefin-edge-node' });
